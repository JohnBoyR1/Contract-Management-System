...
| /api/UserProfiles/ProfileDetails | GET    | Retrieves user profile                                        | Cross-Reference `sub` claim with `{id}` to prevent IDOR attacks                         | Bearer Token (Owner Only) | Id : int                                                                                                                                                                       |
...
| /api/UserAccounts/ChangePassword | PUT    | Updates user credentials                                      | Cross-Reference `sub` claim with `{id}` to prevent IDOR attacks                         | Bearer Token (Owner Only) | Id : int, OldPassword : string, NewPassword : string, ConfirmNewPassword : string                                                                                              |
| /api/UserProfiles/UpdateProfile  | PUT    | Updates user profile                                          | Cross-Reference `sub` claim with `{id}` to prevent IDOR attacks                         | Bearer Token (Owner Only) | Id : int, PhoneNumber : string, Bio : string, AvailableForWork : bool-nullable, OfferingWork : bool-nullable, DisplayUsername : bool-nullable, HidePhoneNumber : bool-nullable |
| /api/UserAccounts/Delete         | DELETE | Deletes user information from database                        | Cross-Reference `sub` claim with `{id}` to prevent IDOR attacks                         | Bearer Token (Owner Only) | Id : int, Password : string                                                                                                                                                    |
